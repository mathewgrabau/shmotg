...
(function ($, ej, doc, undefined) {
    ...
    ej.DataManager.prototype = {
        ...
        executeQuery: function (query, done, fail, always) {
            ...
            if (!this.dataSource.offline && this.dataSource.url != undefined) {
                this._makeRequest(this.adaptor.processQuery(this, query), deffered, args, query);
            } else {
                nextTick(function () {
        ...
        _beforeSend: function (request, settings) {
            ...
            for (var i = 0; headers && i < headers.length; i++) {
                props = [];
                for (var prop in headers[i]) props.push(prop);
                request.setRequestHeader(headers[props[0]], headers[props[1]]);
            }
        },
    ...
    ej.ODataAdaptor = new ej.UrlAdaptor().extend({
        ...
        processResponse: function (data, ds, query, xhr, request, changes) {
            ...
            if (request && request.url.endsWith(this.options.batch) && request.type.toLowerCase() === "post") {
    ...
    ej.JsonAdaptor = new ej.Adaptor().extend({
        ...
        onSortBy: function (ds, e, query) {
            ...
            return stableSort(ds, field, e.comparer);
        },
    ...
    ej.Model.prototype = {
        ...
        bindTo: function (element) {
            var props = this.properties(), el = $(element), ctl, field,
        ...
        unbind: function (element) {
            var props = this.properties(), tmp, data = {
                props: this._attrEls,
        ...
        _updateProps: function (element, value) {
            ...
            for (var prop in value) {
                ...
                if (t in json)
                    t = json[t];
                else if (t in this.computed) {
                    t = this.computed[t];
                    if (t) {
                        t = t.value.call($.extend({}, this.json, this.computed));
                    }
                }
    ...
    ej.Predicate.prototype = {
        ...
        validate: function (record) {
            var value, p = this.predicates, isAnd, ret;
        ...
        merge: function (left, right, fieldName, comparer) {
            ...
            while (left.length > 0 || right.length > 0) {
                if (left.length > 0 && right.length > 0) {
                    if (comparer)
                        current = comparer(getVal(left, fieldName, 0), getVal(right, fieldName, 0)) < 1 ? left : right;
                    else
    ...
    ej.support.stableSort = function () {
        var res = [0, 1, 2, 3, 4].sort(function () { return 0; });
        return res[0] === 0 && res[1] === 1 && res[2] === 2 && res[3] === 3 && res[4] === 4;
    }();
    ...
    var stableSort = function (ds, field, comparer) {
        if (ej.support.stableSort) {
            if(!ej.support.enableLocalizedSort && typeof ej.getObject(field, ds[0] || {}) == "string" 
    ...
    var fastSort = function(ds, field, isDesc){
        ...
        Object.prototype.toString = (field.indexOf('.') === -1) ? function(){
            ...
            return ej.getObject(field, this);
        };
        ds = ds.sort();
